...
func TestStorage_Download(t *testing.T) {
	...
	}{
		{
			...
			setupMock: func(m *MockStorage) {
				m.On("Download", context.Background(), "photos/test.jpg").
					Return([]byte("test image data"), "image/jpeg", nil)
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			data, mimeType, err := mockStorage.Download(context.Background(), tt.key)
...
func TestStorage_GeneratePresignedURL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			setupMock: func(m *MockStorage) {
				m.On("GenerateURL", context.Background(), "photos/test.jpg", 15*time.Minute).
		...
		},
		{
			...
			setupMock: func(m *MockStorage) {
				m.On("GenerateURL", context.Background(), "test.jpg", 5*time.Minute).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			url, err := mockStorage.GenerateURL(context.Background(), tt.key, tt.expiration)
...
func TestStorage_EdgeCases(t *testing.T) {
	...
	t.Run("download empty file", func(t *testing.T) {
		...
		data, mimeType, err := mockStorage.Download(context.Background(), "empty.txt")
	...
	t.Run("presigned URL with very short expiration", func(t *testing.T) {
		...
		mockStorage.On("GenerateURL", context.Background(), "temp.jpg", shortExpiration).
			Return("https://short-lived.example.com/temp.jpg", nil)

		url, err := mockStorage.GenerateURL(context.Background(), "temp.jpg", shortExpiration)
...
func TestStorage_MultipleOperations(t *testing.T) {
	...
	mockStorage.On("GenerateURL", context.Background(), key, 30*time.Minute).
	...
	downloadedData, mimeType, err := mockStorage.Download(context.Background(), key)
	assert.NoError(t, err)
	assert.Equal(t, data, downloadedData)
	assert.Equal(t, "image/jpeg", mimeType)

	presignedURL, err := mockStorage.GenerateURL(context.Background(), key, 30*time.Minute)
	assert.NoError(t, err)
